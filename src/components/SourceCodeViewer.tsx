...
export const SourceCodeViewer = () => {
  // Filter out any unexpected entries (defensive programming)
  const files = sourceFiles.filter(
    (f): f is ProjectFile => !!f && !!f.path && !!f.content
  );
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-hidden bg-muted/20">
            <pre className="h-full overflow-auto p-4 text-sm" style={{ lineHeight: '1.5' }}>
              <code className="font-mono whitespace-pre">{activeFile.content}</code>
            </pre>
          </div>
        </div>
      </div>
...
/* Default export for safety – the named export remains available */
